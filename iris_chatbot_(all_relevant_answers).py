...
KEYWORD_DICTIONARY = {
    "Greeting": ["hi", "hello", "hey", "good morning", "good evening", "whats up", "yo", "sup", "how are you"],
    "STIs": ["STI", "STD", "sexually transmitted infection", "sexually transmitted disease", "sex infection",
             "disease from sex", "can I get an STI", "can I get an STD", "what are STDs", "types of STIs", "common STDs"],
    "STI Symptoms": ["STI symptoms", "STD symptoms", "signs of STI", "signs of STD",
    ...
    "STI Prevention": ["how to prevent STIs", "how to prevent STDs", "safe sex", "protection from STIs",
                       "can condoms prevent STDs", "how to avoid getting an STI", "safe sex tips"],
    "HIV & AIDS": ["HIV", "AIDS", "can I get HIV", "what is HIV", "how does HIV spread", "is HIV curable",
                   "difference between HIV and AIDS", "how to prevent HIV", "can you get HIV from kissing",
                   "does HIV have symptoms", "HIV early symptoms"],
    "Pregnancy Risk": ["pregnancy risk", "can I get pregnant", "pregnancy chance", "will I get pregnant",
                       "am I pregnant", "pregnancy symptoms", "what are the chances of pregnancy",
                       "can sperm on hands cause pregnancy"],
    "Contraception": ["birth control", "contraception", "ways to prevent pregnancy", "condoms", "IUD", "implant",
                      "contraceptive patch", "contraceptive ring", "birth control pills", "how to not get pregnant",
                      "most effective birth control"],
    "Emergency Contraception": ["morning after pill", "plan B", "emergency pill", "after sex pill",
                                "can I still prevent pregnancy", "how late can I take plan B",
                                "how does the morning after pill work"],
    "Menstruation": ["menstruation", "period", "that time of the month", "monthly bleeding",
    ...
    "Adolescence & Puberty": ["adolescence", "puberty", "growing up", "body changes", "why is my voice deeper",
...
# Function to Fetch All Matching Answers
def get_response(user_message):
    ...
    if any(word in user_message for word in KEYWORD_DICTIONARY["Greeting"]):
        return iris_greeting()

    # **Step 1: Exact Match in DB**
    faq_entries = HealthInfo.query.filter(HealthInfo.question.ilike(f"%{user_message}%")).all()

    # **Step 2: Check Keyword-Based Matching if No Exact Match**
    if not faq_entries:
        ...
        if matching_category:
            # Fetch relevant questions from DB that match this category's keywords
            faq_entries = HealthInfo.query.filter(
                db.or_(*[HealthInfo.question.ilike(f"%{kw}%") for kw in KEYWORD_DICTIONARY[matching_category]])
            ).all()

    # **Step 3: Return All Matched Answers**
    if faq_entries:
        # Update FAQ tracking count for each match
        ...
        # Collect and return all relevant answers
        return "\n\n".join([f"- {faq.answer}" for faq in faq_entries])

    # **Step 4: Default Response if No Match Found**
